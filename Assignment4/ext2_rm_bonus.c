...
int main(int argc, char **argv) {
    ...
    else if((get_inode_mode(possible_dup_dir_ent -> inode) & EXT2_S_IFDIR) && !recursive) {
    ...
    if(recursive) {
        free_inode(possible_dup_dir_ent -> inode - 1, true);
    }
    else {
...
void free_inode(int inode, bool recursive) {
    ...
    // File also in indirect blocks, keep  
    if(rm_complete == 0) {
    ...
    if(rm_complete == 0) {
        int *indirect_block = (int *)(disk + (inode_p -> i_block)[14] * EXT2_BLOCK_SIZE);
